...
规则和策略组都是**运行时拉取的**（外配置和规则集进程内缓存 6 小时，重启后重新拉）：
...
| `proxy` | 否 | 直连 | 拉取订阅时走的公开代理，仅支持 `http://` / `https://` / `socks5://`；回环、私网和链路本地代理地址会被拒绝 |
...
- 单次下载最长 30 秒，压缩前和 gzip 解压后的响应均最多 32 MiB；一次 `/sub` 转换从接收请求到返回结果最长 60 秒。
- 最多同时处理 8 个转换请求；超过时返回 `429 Too Many Requests`，并带 `Retry-After: 1`。
  所有订阅、外配置和规则集下载共用最多 16 个并发名额。
- 外配置和规则集缓存最多保留 6 小时、256 条目、128 MiB；超出容量时淘汰最久未使用的内容。
- 服务直接下载时，初始订阅/配置/规则集地址、HTTP 重定向目标，以及实际拨号前重新解析的 DNS 结果，
  都会拒绝回环、私网、链路本地和云 metadata IP，并返回明确的拒绝原因。
- 默认不读取 `HTTP_PROXY` 等环境代理；需要代理时必须通过 `/sub` 的 `proxy` 参数显式指定。
  显式代理本身也会经过地址校验；代理转发后的目标解析与连接由代理服务器执行，不在本服务的拨号前校验范围内。
- 日志不记录 `/sub` 的 query，也不记录远程 URL 的 query；仅保留 host、数量和失败类别。
