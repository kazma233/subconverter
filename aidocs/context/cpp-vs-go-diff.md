...
| 多级缓存 TTL | 订阅缓存 60 秒、配置缓存 300 秒、规则集缓存 6 小时自动刷新，保证规则不过期 | 外配置和规则集缓存 6 小时，最多 256 条/128 MiB；订阅不缓存 |
| serveCacheOnFetchFail | 拉取失败时返回上次缓存的旧内容，而不是直接报错，提升可用性 | 缺失（拉取失败直接报错） |
| 限速 | 限制单次下载大小和规则集数量，防止恶意请求或意外拉取超大文件耗尽内存 | 单次下载压缩前、gzip 解压后均最多 32 MiB；下载并发最多 16 |
...
| 缓存过期 | 规则集 6 小时后自动重新拉取，保证规则是最新的 | 外配置和规则集缓存 6 小时后重新拉取 |
...
| 缓存 TTL | 订阅 60 秒、配置 300 秒、规则集 6 小时自动过期重新拉 | 外配置和规则集缓存 6 小时，最多 256 条/128 MiB；订阅不缓存 |
